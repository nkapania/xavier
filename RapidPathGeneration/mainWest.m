...
load THcenterWest.mat; refWorld.isOpen = 0;  %Just load the centerline path since it has been modified to be continuous
bounds = load('thunderhill_bounds_west.mat');

%%%%%%%%%% HACK - add in a little more buffer at a few parts of the track -
%%%%%%%%%% better way is to link this to EN coordinates from a user
%%%%%%%%%% specified mat file.

buff.s = [0  ; 1747; 2037; refWorld.s(end)];
%buff.b = [.35;  1.2;  1.2;             .35];
buff.b = .05*ones(size(buff.s));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:NUM_ITERS
    ...
    opt  = getRapidTrajectory(refWorld, vpRef, veh, i);
...
%qW = shiftWorld(genWorldFromCSV('C:\Mapping\Maps\Thunderhill West\ThillWestFinal_1m.csv'), 930);
...
close all;
figure;
plot(D(end).world.roadE, D(end).world.roadN,'k');
