...
function [ posDesired, UxDesired, AxDesired, AxMax] = FindSpeedProfile(path, veh, Vmax, sigma,...
    ...
    Vmax = Vmax*ones(1, n);
    posDesired = linspace(path.s(1), path.s(end), n);
    mu   = interp1(path.s, path.mu, posDesired);
    if sum(isnan(mu))
        error('NaN Values Encountered in Interpolating Mu Values')
    end
