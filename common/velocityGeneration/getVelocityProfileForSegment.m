...
function vp = getVelocityProfileForSegment(path, veh, mu, Vmax, brakeFactor, topoFile)
    ...
    if nargin > 5
        ...
        topoData = [[path.s(1); path.s(end)], zeros(2,6)] ;
        
    end
        
    [s, Fv2, G, Mv2, Mvdot, theta] = MakePath3DForSegment(path ,topoData,ds,veh.Iz);


    [ posDesired, UxDesired, AxDesired, AxMax] = FindSpeedProfileForSegment(path, veh, mu, Vmax, s,...
