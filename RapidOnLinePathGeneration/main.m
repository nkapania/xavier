...
useOptimizedLineAsStartingPoint = 0;
...
N_CVX = 95; %50 points for the cvx formulation

lookaheadDist = 800;
STEPCOUNT = 10;
STEPSIZE = STEPCOUNT*floor(lookaheadDist/N_CVX);
TOTALCOUNT = 100;
START = 3.2980e+03; 300;
...
%          end
    ...
    plot(opt.roadE, opt.roadN,'r')%.-','markersize',20)
    legend('Reference Path','New Path','Opt result','Path Segment','Location','North')
    ...
    subplot 222,hold off, plot(refWorld.s(indicesWorld),refWorld.K(indicesWorld),'b'),hold on, stairs(optWorld.s,optWorld.K,'g'),hold on, stairs(opt.s,opt.K,'r')
...
%     xlim([min(opt.s) max(opt.s)])
    ...
    subplot 224,hold off, plot(vpRef.s(indicesVPref),vpRef.Ux(indicesVPref),'b',vpOpt.s(1:end),vpOpt.Ux(1:end),'g-')%,vpOpt.s(1:end),vpOpt.Ux(1:end),'r')
...
%          
        ...
        for i = 1:NUM_ITERS_SRT
            ...
            %% Solve for Optimal Path
            tic
            optSRT  = getRapidOnlineTrajectory(worldSegment, vpSRT, veh,N_CVX);
            toc
            optSRT.isOpen = 1;
            % update world dependent variables e.g. distance to boundary
